...
def train_one_epoch(
    ...
    for i, batch in enumerate(dataloader):
        # print(f"batch {i} of {num_batches_per_epoch}")
        ...
        with autocast():
            ...
            if args.clap_mlploss:
                ...
                total_loss = loss(
                    ...
                    text_features=text_features,
                    logit_scale_a=logit_scale_a,
                    positive_list=torch.Tensor(batch['num_of_pos']),
                    negative_list=torch.Tensor(batch['num_of_neg']),
                )
        if isinstance(optimizer, dict):
    ...
    if args.val_dataset_names == ['Clotho', 'audiocaps']:
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                all_names = list(set([b for b in batch['__url__']]))
...
def evaluate_clotho_audiocaps(
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            all_names = list(set([b for b in batch['__url__']])) #changed 
            for name in all_names:
                if name not in eval_info.keys():
