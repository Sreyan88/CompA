...
class CsvDataset(Dataset):
    def __init__(self, input_filename, img_key, caption_key, sep="\t", is_train = True):
        ...
        # print("$ck self.sim_matrix loading")
        self.sim_matrix = pickle.load(open("././top_5/final_cleaned_top_k.pkl", "rb"))
        # df = pd.DataFrame.from_dict({"path": self.sim_matrix.keys()})
        # df.to_csv("././sim__comp_paths.csv",index=False)
        self.csv_dict = pickle.load(open("././csv_dict.pkl", "rb"))
        # print("$ck self.sim_matrix loaded")
        self.k = 5
        self.count = 0
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            if self.images[idx] not in self.sim_matrix:
                rand_k_idx = random.randint(0, len(self.images)-1)
                # print(f"Idx : {idx}, top_k_idx : {rand_k_idx}")
                is_train = "train"
                ...
                data_dict = {
                    ...
                    "alt_audio": torchaudio.load(self.images[rand_k_idx]),
                    "alt_raw_text": str(self.captions[rand_k_idx]),
                    "alt_path": self.images[rand_k_idx], 
                    "alt__url__": str(self.dataset_names[rand_k_idx]) + "-" +  "train" if self.is_train else str(self.split_column[rand_k_idx])
                }
            else:
                rand_key = ""
                temp_list = []
                for key in self.sim_matrix[self.images[idx]].keys():
                    if key in self.csv_dict.keys():
                        temp_list.append(key)
                if len(temp_list) == 0:
                    rand_k_idx = random.randint(0, len(self.images))
                    # print(f"Idx : {idx}, top_k_idx : {rand_k_idx}")
                    is_train = "train"
                    ...
                    rand_key = random.choice(temp_list)
                    # print(f"Idx : {idx}, top_k : {rand_key}")
                    is_train = "train"
                    ...
                    data_dict = { 
                        ...
                        "raw_text": text,
                        "__url__": name + "-" +  is_train,
                        "alt_audio": torchaudio.load(rand_key),
                        "alt_path": rand_key, 
                        "alt_raw_text": str(self.csv_dict[rand_key]["caption"]),
                        "alt__url__": str(self.csv_dict[rand_key]["dataset"]) + "-" +  "train" if self.is_train else str(self.csv_dict[rand_key]["split_name"])
                    }
        else:
...
def preprocess_single(
    ...
    #for negatives
    text_list = [tokenizer(x, tmodel=tmodel) for x in sample["raw_text"].split(';')]  # text shape: [num_token]
    concatenated_tensors = {}
    for text in text_list:
        for key, tensor in text.items():
            if key in concatenated_tensors:
                if len(concatenated_tensors[key].shape) > 1:
                    concatenated_tensors[key] = torch.cat([concatenated_tensors[key], tensor.unsqueeze(0)], dim=0)
                else:
                    concatenated_tensors[key] = torch.cat([concatenated_tensors[key].unsqueeze(0), tensor.unsqueeze(0)], dim=0)    
            else:
                concatenated_tensors[key] = tensor    

    sample["text"] = concatenated_tensors
...
def preprocess_single_alt(
    ...
    Preprocess a single sample for wdsdataloader.
    """
    sample["audio"] =  sample["alt_audio"]
    sample["raw_text"] =  sample["alt_raw_text"]
    sample["__url__"] =  sample["alt__url__"]
...
def collate_fn_with_preprocess(batch,
    ...
    for sample in batch:
        assert len(sample["raw_text"].split(";")) == 4, f"Mismatch error for {sample['path'] + ' ' + sample['raw_text'] +' $$'}"
        assert len(sample["alt_raw_text"].split(";")) == 4, f"Mismatch error for {sample['path'] + ' $ ' + sample['alt_path'] + ' $ ' + sample['alt_raw_text'] +' $$'}"
        data_preprocessed.append(
            preprocess_single(sample, max_len, audio_cfg, tmodel, class_index_dict, data_filling,
                              data_truncating))
        if is_train:
            data_preprocessed.append(
                preprocess_single_alt(sample, max_len, audio_cfg, tmodel, class_index_dict, data_filling,
                                        data_truncating))
